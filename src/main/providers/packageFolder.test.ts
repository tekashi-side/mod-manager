...
describe('PackageFolderProvider', () => {
  ...
  it('marks mods in package/disabled as disabled', async () => {
    ...
    expect(list).toContainEqual({ modId: 'Foo', version: 2, fileName: 'uisciasFoo_2.it', enabled: true })
    expect(list).toContainEqual({ modId: 'Bar', version: 3, fileName: 'uisciasBar_3.it', enabled: false })
