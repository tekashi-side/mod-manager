...
### 4. Hand-vendored `carousel.tsx` (no Luma variant exists)

`components/ui/carousel.tsx` is **not** generated by our Luma preset. The `radix`
style registry has no carousel — `https://ui.shadcn.com/r/styles/radix/carousel.json`
returns **404**. The carousel is only published under the classic styles (e.g.
`new-york-v4`), so there was nothing to `apply`/`add` for our style. It's vendored by
hand from the classic component (adds the `embla-carousel-react` dependency) and
carries one deliberate deviation from stock:

- **Arrow centering uses `top-[calc(50%-1rem)]`, not `-translate-y-1/2`.** The stock
  carousel centers its nav buttons with `top-1/2 -translate-y-1/2`. Our Luma `Button`
  has `active:translate-y-px`, and both write the same `--tw-translate-y` variable —
  so on click the press would clobber the `-50%` centering and the button jumps most
  of its own height. The `calc` offset (`size-8` = 2rem, so half is `1rem`) centers
  without a transform, leaving `active:translate-y-px` free to apply the intended 1px
  press. `CarouselPrevious`/`CarouselNext` are affected (both orientations).

Full-bleed image behavior (dropping the `-ml-4`/`pl-4` inter-slide gutter via
`ml-0`/`pl-0`) lives at the call site in
[`ModDetail.tsx`](../src/renderer/components/ModDetail.tsx), not in the primitive.

**On reinstall / re-theme:** the CLI won't touch this file (it doesn't ship a Luma
carousel), but if you ever `add carousel` for a classic style, it will pull the stock
`-translate-y-1/2` version — re-apply the `calc` centering above, or the press jump
returns.
